...
// --- Overeating / "stuffed" debuffs (ported from Azure-Peak PR #7706) -----------------------------
// Eating/drinking too many stacking buffs at once (see process_buffstacks in roguebuff.dm) escalates
// you through these tiers. Alert icons live in our screen_alert.dmi (states fatchud / fatchud_cholesterol
// / fatchud_chungus). ES note: AP also applied TRAIT_ARMOR_BREAK at tier 3, but ES has no such trait/
// mechanic, so only TRAIT_CRITICAL_WEAKNESS is applied here.
...
// Why isn't this in on_apply, checked once? Because chemplayers would absolutely figure out they can get
// stuffed first, -then- drink the potion. Never presume you've outsmarted them.
/datum/status_effect/debuff/stuffed_one/process(wait)
	. = ..()
...
/datum/status_effect/debuff/stuffed_two/process(wait)
	. = ..()
	// We kind of have to do this because these reagents themselves handle their effects.
	var/datum/reagents/reag = owner.reagents
	if(reag)
