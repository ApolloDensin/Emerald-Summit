...
/obj/item/patronage_writ/examine(mob/user)
	. = ..()
	if(issuer_year)
		. += span_info("Signed by [issuer_name || "an unknown hand"], [issuer_year]. Use in hand to claim agency of [faction_label].")
	else
		. += span_info("Signed by [issuer_name || "an unknown hand"]. Use in hand to claim agency of [faction_label].")
...
/obj/item/patronage_writ/benefactor
	...
	desc = "A sealed letter of the Church, conferring the rank of Benefactor upon its bearer."
