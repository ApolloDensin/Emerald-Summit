...
/obj/structure/roguemachine/steward/ui_data(mob/user)
	...
	for(var/cat_id in GLOB.petition_categories)
		...
		for(var/region_id in GLOB.economic_regions)
			per_region[region_id] = SSeconomy.petition_blocker(region_id, cat_id) || ""
...
/obj/structure/roguemachine/steward/ui_act(action, list/params)
	...
	if(SStreasury.is_in_receivership() && (action in GLOB.steward_trade_sequestration_locked_actions))
		to_chat(usr, span_warning("The Emerald Trading Company holds the Crown's commerce in sequestration. Petition, tax, and fine are your remaining instruments."))
		return TRUE
	...
	switch(action)
		...
		if("take_atc_loan")
			...
			if(SStreasury.take_atc_loan(amount, usr))
				playsound(src, 'sound/items/inqslip_sealed.ogg', 70, FALSE, -1)
				visible_message(span_notice("[src] stamps a sealed writ. The wax bears the mark of the Emerald Trading Company."))
			SStgui.update_uis(src)
			return TRUE
