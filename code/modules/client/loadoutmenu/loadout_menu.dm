...
/datum/loadout_menu/ui_static_data(mob/user)
	...
	// Emerald port: GLOB.loadout_items is `path -> datum` assoc; iterating gives keys (paths). Ratwood's
	// code accidentally called procs on the typepath, which throws and kills the loop. Fetch the datum.
	for(var/path as anything in GLOB.loadout_items)
		...
		var/donoritem_passed = TRUE // True means it won't show up in the TGUI
		var/noble_passed = item.nobility_check(user.client)
		if(item.donoritem)
			if(!item.donator_ckey_check(user.key))
				donoritem_passed = FALSE
		UNTYPED_LIST_ADD(loadout_items, list(
			"name" = item.name,
			"desc" = initial(I.desc),
			"triumph_cost" = item.desc, // Don't @ me... this is wack.
			"nobility_check" = noble_passed,
