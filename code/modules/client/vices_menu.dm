...
/datum/preferences/proc/save_to_history()
	...
	// Emerald port: dedupe at insert. Many menu actions fire save_to_history at the moment the user clicks
	// "Change X" — before any actual change. If we kept all those, the counter inflates and undo becomes a no-op.
	if(customization_history.len && _snapshot_matches_current(customization_history[1]))
		return

	// Emerald port: prepend snapshot. We use LIST_VALUE_WRAP_LISTS + `+` because BYOND's list.Insert(pos, list_arg)
	// flattens the list arg and inserts its elements one by one, which would split each snapshot into ~50 key/value
	// fragments. Ratwood ships this bug; the wrap keeps each snapshot as a single history entry.
	customization_history = LIST_VALUE_WRAP_LISTS(snapshot) + customization_history

	// Keep only last 10 entries
	if(customization_history.len > 10)
		customization_history.Cut(11)

/datum/preferences/proc/_snapshot_matches_current(list/snapshot)
	// Emerald port helper: returns TRUE if the given snapshot is functionally identical to current prefs state.
	// Used to skip no-op undos when save_to_history fired on a "Change X" click that the user cancelled out of.
	if(!snapshot)
		return FALSE
	if(snapshot["statpack"] != statpack)
		return FALSE
	if(snapshot["virtue"] != virtue)
		return FALSE
	if(snapshot["virtuetwo"] != virtuetwo)
		return FALSE
	for(var/i = 1 to 5)
		if(snapshot["vice[i]"] != vars["vice[i]"])
			return FALSE
	for(var/i = 1 to 10)
		var/slot_var = (i == 1) ? "loadout" : "loadout[i]"
		if(snapshot[slot_var] != vars[slot_var])
			return FALSE
		if(snapshot["loadout_[i]_name"] != vars["loadout_[i]_name"])
			return FALSE
		if(snapshot["loadout_[i]_desc"] != vars["loadout_[i]_desc"])
			return FALSE
		if(snapshot["loadout_[i]_hex"] != vars["loadout_[i]_hex"])
			return FALSE
	if(snapshot["extra_language"] != extra_language)
		return FALSE
	if(snapshot["extra_language_1"] != extra_language_1)
		return FALSE
	if(snapshot["extra_language_2"] != extra_language_2)
		return FALSE
	return TRUE
...
/datum/preferences/proc/undo_last_change()
	...
	// Skip any leading no-op snapshots (saved on "Change X" clicks the user cancelled out of).
	while(customization_history.len && _snapshot_matches_current(customization_history[1]))
		customization_history.Cut(1, 2)
	if(!customization_history.len)
		return FALSE
...
/datum/preferences/proc/generate_vices_html(mob/user)
	...
	// FREE LANGUAGE SLOT — Emerald port: Ratwood uses `origin`, Emerald's equivalent is `virtue_origin`.
	// Cast to /datum/virtue/origin because virtue_origin is declared as the base /datum/virtue,
	// and origin_language only exists on the origin subtype.
	var/datum/virtue/origin/origin_datum = virtue_origin
	var/origin_lang = istype(origin_datum) ? origin_datum.origin_language : null
	...
	if(origin_lang)
		...
		html += "<div class='vice-desc'>Granted by your origin ([virtue_origin.name]). Cannot be changed.</div>"
		html += "</div></div></div>"
	else
...
// Emerald port: was /datum/preferences/Topic in Ratwood — renamed to a helper so it coexists with
// Emerald's existing Topic handler. Called from /datum/preferences/Topic in preferences.dm.
// Returns TRUE if the click was handled, FALSE otherwise.
/datum/preferences/proc/handle_vices_menu_topic(href, list/href_list)
	...
	if(href_list["virtue_action"])
		...
		if(action == "change_primary")
			...
			for(var/path as anything in GLOB.virtues)
				...
				if(!V.name)
					continue
				// Emerald port: mirror the classic prefs.dm virtue filter — exclude origins (those grant
				// starting languages and belong on the Languages tab), packs, racials/heretics not for this char.
		...
		if(action == "change_secondary")
			...
			for(var/path as anything in GLOB.virtues)
				...
				if(!V.name)
					continue
				// Emerald port: mirror primary picker's filter — exclude origins (Languages tab) and packs.
				if(istype(V, /datum/virtue/origin))
					continue
				if(istype(V, /datum/virtue/pack))
					continue
				...
				if(virtue && V.type == virtue.type)
					continue
				// Check for conflicting vices — silent during list-building.
				if(check_virtue_vice_conflict(V.type, FALSE, usr))
					continue
				// Check for conflicting virtues (with primary virtue) — silent during list-building.
				if(virtue && check_virtue_virtue_conflict(V.type, virtue.type, FALSE, usr))
					continue
				virtues_available[V.name] = V
		...
		switch(action)
			if("select", "change")
				...
				for(var/vice_name in GLOB.character_flaws)
					...
					// Check for conflicting virtues — silent during list-building (we just filter; the user can't pick what's not listed).
					if(check_vice_virtue_conflict(vice_type, FALSE, usr))
						continue

					// Check for conflicting vices (eye-related) — silent for the same reason.
					if(check_vice_vice_conflict(vice_type, selected_vices, FALSE, usr))
						continue
				...
				if(choice)
					...
					// Emerald port: keep legacy `charflaw` in sync with vice1. Several legacy paths (Class Selection's
					// SetChoices, job restriction checks, examine, etc.) call `charflaw.type` / `charflaw.name` directly
					// and runtime-error if it's null. Ratwood's `charflaw = null` here breaks those.
					if(slot == 1)
						charflaw = new_vice
					else if(!charflaw && vice1)
						charflaw = vice1
	...
	if(href_list["language_action"])
		...
		if(action == "free_select" || action == "free_change")
			// Emerald port: dropped /datum/language/canilunzt, /datum/language/merar, and
			// /datum/language/thievescant/signlanguage from Ratwood's list — they don't exist here.
