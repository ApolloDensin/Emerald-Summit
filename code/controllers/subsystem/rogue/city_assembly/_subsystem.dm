...
/datum/controller/subsystem/city_assembly/proc/resolve_session(source = "unknown", flavor_prefix = null, skip_quorum = FALSE)
	...
	// Poll tax disabled pending anti-dodge design (see AP docs/design/estates_general_stage1.md).
...
/datum/controller/subsystem/city_assembly/proc/flatten_summary_html(text)
	if(!text)
		return ""
	var/out = text
	out = replacetext(out, "<br>", " -- ")
	out = replacetext(out, "<b>", "")
	out = replacetext(out, "</b>", "")
	out = replacetext(out, "<i>", "")
	out = replacetext(out, "</i>", "")
	out = replacetext(out, "&mdash;", "-")
	return out
